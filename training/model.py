...
class Model(nn.Module):
    ...
    def load_the_model(self, filename='models/latest.pt'):
        try:
            # Load weights with proper device mapping for MPS
            if torch.backends.mps.is_available():
                self.load_state_dict(torch.load(filename, map_location='mps'))
            elif torch.cuda.is_available():
                self.load_state_dict(torch.load(filename, map_location='cuda:0'))
            else:
